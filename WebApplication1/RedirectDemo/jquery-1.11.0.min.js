 ...
 * @version 1.4.12
 */
(function(A){if(typeof define==="function"&&define.amd){define(["jquery"],A)}else{A(jQuery)}}(function(C){var B=C.scrollTo=function(F,E,D){return C(window).scrollTo(F,E,D)};B.defaults={axis:"xy",duration:parseFloat(C.fn.jquery)>=1.3?0:1,limit:true};B.window=function(D){return C(window)._scrollable()};C.fn._scrollable=function(){return this.map(function(){var D=this,E=!D.nodeName||C.inArray(D.nodeName.toLowerCase(),["iframe","#document","html","body"])!=-1;if(!E){return D}var F=(D.contentWindow||D).document||D.ownerDocument||D;return/webkit/i.test(navigator.userAgent)||F.compatMode=="BackCompat"?F.body:F.documentElement})};C.fn.scrollTo=function(F,E,D){if(typeof E=="object"){D=E;E=0}if(typeof D=="function"){D={onAfter:D}}if(F=="max"){F=9000000000}D=C.extend({},B.defaults,D);E=E||D.duration;D.queue=D.queue&&D.axis.length>1;if(D.queue){E/=2}D.offset=A(D.offset);D.over=A(D.over);return this._scrollable().each(function(){if(F==null){return}var G=this,L=C(G),K=F,I,J={},N=L.is("html,body");switch(typeof K){case"number":case"string":if(/^([+-]=?)?\d+(\.\d+)?(px|%)?$/.test(K)){K=A(K);break}K=N?C(K):C(K,this);if(!K.length){return}case"object":if(K.is||K.style){I=(K=C(K)).offset()}}var M=C.isFunction(D.offset)&&D.offset(G,K)||D.offset;C.each(D.axis.split(""),function(S,P){var R=P=="x"?"Left":"Top",T=R.toLowerCase(),V="scroll"+R,O=G[V],Q=B.max(G,P);if(I){J[V]=I[T]+(N?0:O-L.offset()[T]);if(D.margin){J[V]-=parseInt(K.css("margin"+R))||0;J[V]-=parseInt(K.css("border"+R+"Width"))||0}J[V]+=M[T]||0;if(D.over[T]){J[V]+=K[P=="x"?"width":"height"]()*D.over[T]}}else{var U=K[T];J[V]=U.slice&&U.slice(-1)=="%"?parseFloat(U)/100*Q:U}if(D.limit&&/^\d+$/.test(J[V])){J[V]=J[V]<=0?0:Math.min(J[V],Q)}if(!S&&D.queue){if(O!=J[V]){H(D.onAfterFirst)}delete J[V]}});H(D.onAfter);function H(O){L.animate(J,E,D.easing,O&&function(){O.call(this,K,D)})}}).end()};B.max=function(G,J){var E=J=="x"?"Width":"Height",I="scroll"+E;if(!C(G).is("html,body")){return G[I]-C(G)[E.toLowerCase()]()}var D="client"+E,F=G.ownerDocument.documentElement,H=G.ownerDocument.body;return Math.max(F[I],H[I])-Math.min(F[D],H[D])};function A(D){return C.isFunction(D)||typeof D=="object"?D:{top:D,left:D}}return B}));
